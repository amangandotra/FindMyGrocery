...
static void rfid_ui_issue_callback(const char *uid);
...
static void reset_issue_scan_ui(void)
{
    rfid_clear_issue_uid();
...
static void rfid_ui_issue_callback(const char *uid)
{
    /* this runs in LVGL task context (because rfid_service used lv_async_call) */
    ESP_LOGI("ISSUE_RFID", "UI callback got UID: %s", uid);

    /* if already captured, ignore */
    if (g_issue_rfid_uid[0] != '\0') {
        ESP_LOGI("ISSUE_RFID", "already captured, ignoring");
        return;
    }

    /* save */
    strncpy(g_issue_rfid_uid, uid, sizeof(g_issue_rfid_uid) - 1);
    g_issue_rfid_uid[sizeof(g_issue_rfid_uid)-1] = '\0';
...
static void issue_scan_screen_event_cb(lv_event_t * e)
{
    lv_event_code_t code = lv_event_get_code(e);

    if(code == LV_EVENT_SCREEN_LOADED) {

        ESP_LOGI("RFID_UI", "Issue Scan screen loaded");
        ...
        rfid_set_callback(on_issue_rfid_scanned);
        rfid_set_mode(RFID_MODE_ISSUE);
    }

    else if(code == LV_EVENT_SCREEN_UNLOADED) {

        ESP_LOGI("RFID_UI", "Issue Scan screen unloaded");
...
void ui_screenIssueScan_screen_init(void)
{
    ...
    lv_obj_add_event_cb(ui_screenIssueScan, issue_scan_screen_event_cb, LV_EVENT_ALL, NULL);
